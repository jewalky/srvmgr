...
void __stdcall log_format(char *s, ...){///TESTED100%, STDCALL
	char line[BUF_MAX], l2[BUF_MAX];
	va_list va;
	va_start(va, s);
	_vsnprintf(line, BUF_MAX-1, s, va);
	va_end(va);
	SYSTEMTIME tm;
	GetLocalTime(&tm);
	_snprintf(l2, BUF_MAX-1, "[%d.%.2d.%.4d %d:%.2d:%.2d.%.3d] %s", tm.wDay, tm.wMonth, tm.wYear, tm.wHour, tm.wMinute, tm.wSecond, tm.wMilliseconds, line);
	to_encoding(l2);
...
{
	if (encoding == KOI8) from_koi(s);
	if (encoding == CP1251) from_win(s);
}

void from_encoding(std::string &s)
{
	if (encoding == KOI8) from_koi(const_cast<char *>(s.c_str()));
	if (encoding == CP1251) from_win(const_cast<char *>(s.c_str()));
}

void to_encoding(std::string &s)
{
	if (encoding == KOI8) to_koi(const_cast<char *>(s.c_str()));
	if (encoding == CP1251) to_win(const_cast<char *>(s.c_str()));
}

void to_encoding(char *s)
{
	if (encoding == KOI8) to_koi(s);
	if (encoding == CP1251) to_win(s);
}
...
void to_win(char *s) 
{
	static const unsigned char table[] = 
	{
		...
		0xE0, 0xE1, 0xE2, 0xE3, 0xE4, 0xE5, 0xE6, 0xE7, 0xE8, 0xE9, 0xEA, 0xEB, 0xEC, 0xED, 0xEE, 0xEF,
		0x3F, 0xB1, 0xB2, 0xB3, 0xB4, 0x3F, 0xB6, 0x3F, 0x3F, 0x3F, 0x3F, 0xBB, 0xBC, 0xBD, 0xBE, 0xAC,
		0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x86, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F,
		0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x87, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F,
		0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF,
		0xA8, 0xB8, 0xAA, 0xBA, 0xAF, 0xBF, 0xA1, 0xA2, 0xB0, 0x3F, 0xB7, 0x3F, 0xB9, 0xA4, 0x3F, 0xA0,
	};
	while (*s) 
	{
		*s = static_cast<char>(table[static_cast<unsigned char>(*s)]);
